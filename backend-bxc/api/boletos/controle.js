...
module.exports = async (req, res) => {
  ...
  try {
    // Extrair numero_controle do query parameter (via vercel.json rewrite)
    const url = new URL(req.url, `http://${req.headers.host}`);
    const numero_controle = url.searchParams.get('numero_controle');

    console.log(`🚀 API Boletos/Controle Request: ${req.method} ${req.url}`);
    console.log(`🔍 Query params:`, Object.fromEntries(url.searchParams));
    console.log(`📋 Número Controle extraído: ${numero_controle}`);

    if (!numero_controle || numero_controle === 'controle') {
      console.log('❌ Número de controle não fornecido');
